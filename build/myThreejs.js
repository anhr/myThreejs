...
function create(create3Dobjects, options) {
	...
	function onloadScripts() {
		...
		var camera,
		    defaultCameraPosition = new three_module_js.Vector3(0.4, 0.4, 2),
		    scene,
		    renderer,
		    controls,
		    stereoEffect,
		    ...
		    axesHelper,
		    colorsHelper = 0x80;
		var mouse = new three_module_js.Vector2(),
		intersects;
		...
		animate();
		function init() {
			camera = new three_module_js.PerspectiveCamera(70, window.innerWidth / window.innerHeight, 0.01, 10);
			camera.position.copy(defaultCameraPosition);
			scene = new three_module_js.Scene();
			scene.background = new three_module_js.Color(0x000000);
			scene.fog = new three_module_js.Fog(0x000000, 250, 1400);
			renderer = new three_module_js.WebGLRenderer({
				antialias: true,
				canvas: elCanvas
			});
			...
			var light = new three_module_js.PointLight(0xffffff, 1);
			light.position.copy(new three_module_js.Vector3(1, 1, 1));
			scene.add(light);
			light = new three_module_js.PointLight(0xffffff, 1);
			light.position.copy(new three_module_js.Vector3(-2, -2, -2));
			scene.add(light);
			group = new three_module_js.Group();
			scene.add(group);
			...
			if (options.axesHelper) {
				axesHelper = new three_module_js.AxesHelper(1 * scale, {
					cookie: cookie,
					scene: scene,
					negativeAxes: true,
			...
			if (gui !== undefined) {
				...
				dat.controllerNameAndTitle(gui.add({
					defaultF: function defaultF(value) {
						controls.target = new three_module_js.Vector3();
						camera.position.copy(defaultCameraPosition);
			...
			group.children.forEach(function (item) {
				if (item.userData.raycaster !== undefined) {
					if (raycaster === undefined) {
						raycaster = new three_module_js.Raycaster();
		...
		function onDocumentMouseMove(event) {
			if (raycaster.stereo !== undefined) raycaster.stereo.onDocumentMouseMove(event);else {
				...
				var left = renderer.domElement.offsetLeft,
				    ...
				    size = new three_module_js.Vector2();
				renderer.getSize(size);
...
function addSpriteTextIntersection(raycaster, intersection, scene) {
	...
	spriteTextIntersection = new three_module_js.SpriteText('x: ' + position.x + ' y: ' + position.y + ' z: ' + position.z, {
		textHeight: 0.1,
		fontColor: textColor,
		rect: {
			displayRect: true,
		...
		center: new three_module_js.Vector2(0.5, 0)
	});
...
switch (lang_js.getLanguageCode()) {
	case 'ru':
...
exports.addSpriteTextIntersection = addSpriteTextIntersection;
exports.removeSpriteTextIntersection = removeSpriteTextIntersection;
